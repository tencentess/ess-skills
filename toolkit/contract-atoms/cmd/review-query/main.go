...
func main() {
	fs := flag.NewFlagSet("review-query", flag.ExitOnError)
	...
	request := ess.NewDescribeContractReviewTaskRequest()
	request.Operator = essClient.Operator()
	request.TaskId = common.StringPtr(taskID)

	response, err := essClient.SDKClient.DescribeContractReviewTask(request)
	if err != nil {
